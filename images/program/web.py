...
def layout_parkir():
    ...
    if len(table_parkir) > 0 :
        ...
        if last["waktu_keluar"] == None:
            waktu_masuk = datetime.datetime.strptime(str(
              str(last["waktu_masuk"]), '%Y-%m-%d %H:%M:%S')
            ...
            waktu_keluar = datetime.datetime.strptime(str(
              last["waktu_keluar"]), '%Y-%m-%d %H:%M:%S')
    ...
    return render_template('index.html', tempat_parkir = 
      ...
      text_keterangan, posisi_parkir=posisi_parkir, plat=plat, 
      saldo=saldo, time = str(datetime.datetime.now()))
...
def save_table_parkir(kendaraan, tempat_parkir, 
    ...
    if waktu_masuk != "NULL":
        ...
        cur.execute(f"insert into parkir(id_rfid_tag, nomor_plat, 
          id_tempat_parkir, waktu_masuk, waktu_keluar) values 
          ('{id_rfid_tag}', '{nomor_plat}', '{id_tempat_parkir}', 
          {waktu_masuk}, NULL) ;")

    # keluar
    elif waktu_keluar != "NULL":
        cur.execute(f"update parkir set waktu_keluar={waktu_keluar} 
          where id_rfid_tag={id_rfid_tag} AND id_tempat_parkir=
          '{tempat_parkir}' AND waktu_keluar is NULL;")
...
def save_table_kendaraan(kendaraan):
    ...
    cur = mysql.connection.cursor()
    cur.execute(f"insert into kendaraan(nomor_plat, id_rfid_tag) 
      values ('{nomor_plat}', '{id_rfid_tag}');")
...
def api_masuk():
    ...
    # update reload id
    cur = mysql.connection.cursor()
    cur.execute("update reload_id set uid = %s where id = 1;", 
      (kendaraan["id_rfid_tag"],))
    ...
    if cek_keluar:
        ...
        for data in table_rfid_tag:
            if data["id"] == kendaraan["id_rfid_tag"]:
                saldo = data["saldo"]
                if saldo < tarif :
                    r = {'status':'saldo tidak mencukupi'}
                    return jsonify(r)
        ...
        for data in table_parkir:
            if data["id_rfid_tag"] == kendaraan["id_rfid_tag"] 
              ...
              data["nomor_plat"] == kendaraan["nomor_plat"]:

                tempat_parkir.append(data)
                break
        
        # update status tempat parkir di table tempat_parkir
        # menjadi tersedia
        update_status_tempat_parkir(
          tempat_parkir[0]['id_tempat_parkir'], "Tersedia")

        # update waktu keluar
        waktu_keluar = "now()"
        save_table_parkir(kendaraan, 
          tempat_parkir[0]['id_tempat_parkir'], 
    ...
    # Parkir Masuk
        ...
        tempat_parkir = tentukan_tempat_parkir(table_tempat_parkir,
...
def hitung_tarif(kendaraan, table_parkir):
    ...
    for data in table_parkir:
        for tempat_parkir in table_tempat_parkir:
            if data["id_rfid_tag"] == kendaraan["id_rfid_tag"] and 
              data["waktu_keluar"] == None and 
              data["id_tempat_parkir"] == tempat_parkir["id"]: 
...
def hitung_tarif2(kendaraan, data, table_tempat_parkir):
    ...
    waktu_masuk = datetime.datetime.strptime(
      str(data["waktu_masuk"]), '%Y-%m-%d %H:%M:%S')
...
def update_saldo(kendaraan, tarif):
    ...
    cur = mysql.connection.cursor()
    cur.execute(f"select saldo from rfid_tag where 
      id = {id_rfid};")

    saldoAwal = cur.fetchall()

    saldoSekarang = saldoAwal[0][0] - int(tarif)
    print('saldo sekarang ', saldoSekarang)

    cur.execute(f"update rfid_tag set saldo={saldoSekarang} 
      where id={id_rfid};")
...
def isi_data():
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute(f"insert into rfid_tag(id, jenis_kendaraan, 
          saldo) values ({idRFID}, '{jenis_kendaraan}', {saldo});")
...
def insert():
    if request.method == "POST":
        ...
        kendaraan = {"id_rfid_tag":id_rfid_tag, 
        ...
        tempat_parkir = tentukan_tempat_parkir(
          table_tempat_parkir,jenis_kendaraan)
...
def delete(id_data):
    ...
    cur = mysql.connection.cursor()
    cur.execute(f"DELETE FROM parkir where id = '{id_data}';")
    mysql.connection.commit()
    return redirect(url_for('table_parkir'))
